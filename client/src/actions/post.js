  ...
  return async (dispatch) => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}api/posts/search?search=${search}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  return async (dispatch) => {
    try {
      ...
      if (
        ...
        "News with this pubDate already exists in DataBase"
      ) {
        alert("Новина за такою датою вже існує в БД");
      }
      console.error(e);
    } finally {
      dispatch(hidePreloader());
