...
export default function postReducer(state = defaultState, action){
    switch(action.type){
        ...
        case EMPTY_POSTS: 
            return {...state, posts: []}                
        case ADD_POST: 
            return {...state, posts: [action.payload, ...state.posts], totalCount: state.totalCount+1}        
        case DELETE_POST:            
            return {...state, posts: [...state.posts.filter(post => post._id !== action.payload)], totalCount: state.totalCount-1}                
        case SET_POSTSCOUNT:            
